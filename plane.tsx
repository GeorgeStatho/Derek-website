...
export function applyPlaneNormalizedPosition(plane: HTMLElement) {
...
function setupPlaneDrag(plane: HTMLElement) {
  ...
  plane.addEventListener("animationiteration", (e: AnimationEvent) => {
  // Ignore bubbled animation events from child SVG parts (like .stream pulse)
  if (e.target !== plane) return;

  // Only handle the post-drag upward animation loop
  if (e.animationName !== "plane-drift-up") return;
  if (!resumeMode || !plane.classList.contains("plane-resume")) return;
