...
let planePosNorm = null;
export function applyPlaneNormalizedPosition(plane) {
    if (!planePosNorm)
        return;
...
function setupPlaneDrag(plane) {
    ...
    // offsets stored in DESIGN-space coords
    let offsetXDesign = 0;
    let offsetYDesign = 0;
    ...
    const onPointerUp = () => {
        ...
        // Keep dragged left/top (do NOT clear anchors)
        // Start/resume the "fly upward from here" mode
        plane.style.animation = "none";
        plane.style.transform = "none";
        plane.classList.remove("plane-resume");
        // Force reflow so the animation reliably restarts from translateY(0)
        void plane.getBoundingClientRect();
        plane.classList.add("plane-resume");
        resumeMode = true;
    };
    plane.addEventListener("pointerdown", (e) => {
        ...
        // Pointer in same coordinate system (design coords)
        const pointerX = (e.clientX - pageRect.left) / scale;
        const pointerY = (e.clientY - pageRect.top) / scale;
        offsetXDesign = pointerX - currentLeft;
        offsetYDesign = pointerY - currentTop;
        // Disable CSS motion while dragging
        plane.classList.remove("plane-resume");
        resumeMode = false;
        plane.style.animation = "none";
        plane.style.transform = "none";
        // Freeze into left/top at exact visible position
    ...
    plane.addEventListener("animationiteration", (e) => {
        // Ignore bubbled animation events from child SVG parts (like .stream pulse)
        if (e.target !== plane)
            return;
        // Only handle the post-drag upward animation loop
        if (e.animationName !== "plane-drift-up")
            return;
        if (!resumeMode || !plane.classList.contains("plane-resume"))
            return;
        // Keep same X (left), reset Y to bottom spawn
        const currentLeft = plane.style.left;
        plane.style.left = currentLeft;
        plane.style.right = "auto";
        plane.style.top = `${DESIGN_HEIGHT + 120}px`; // respawn below page
        plane.style.bottom = "auto";
        // Keep normalized position in sync (optional but useful for resize)
        const leftPx = parseFloat(plane.style.left || "0");
        const topPx = parseFloat(plane.style.top || "0");
        if (!Number.isNaN(leftPx) && !Number.isNaN(topPx)) {
            planePosNorm = {
                x: leftPx / DESIGN_WIDTH,
                y: topPx / DESIGN_HEIGHT,
            };
        }
    });
}
